...
基于 LangGraph 的 Agentic RAG —— 不是"检索一次直接回答"，而是让 LLM 像研究员一样：主动拆解问题、多轮检索、分析对比、交叉验证，最后汇总成一份有来源支撑的回答。

和普通 RAG 的区别：

| | 普通 RAG | Agentic RAG（本系统） |
|---|---|---|
| 检索次数 | 1 次，取 top-k 拼接 | 多次迭代，根据中间结果决定下一步 |
| 问题处理 | 原样检索 | LLM 先改写/拆解，再逐个子问题检索 |
| 决策能力 | 无 | Agent 自主判断：信息够不够、要不要换关键词 |
| 答案来源 | 单个 chunk 拼贴 | 多文档综合分析，标注来源
