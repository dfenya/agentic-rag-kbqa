...
async def create_conversation(body: ConversationCreateRequest, request: Request):
    ...
    conv = container.conversation_service.create(
...
@router.get("")
async def list_conversations(q: str | None = None, request: Request = None):
    container = get_container(request)
    convs = container.conversation_service.list_conversations(q=q)
    return {"items": [
        ConversationResponse(
...
async def get_messages(conv_id: str, request: Request):
    container = get_container(request)
    msgs = container.conversation_service.get_messages(conv_id)
    result = []
    for m in msgs:
        flow_steps = None
...
async def update_conversation(conv_id: str, body: ConversationUpdateRequest, request: Request):
    container = get_container(request)
    conv = container.conversation_service.update(conv_id, **(body.model_dump(exclude_none=True)))
    if not conv:
...
async def delete_conversation(conv_id: str, request: Request):
    container = get_container(request)
    ok = container.conversation_service.delete(conv_id)
    if not ok:
        raise HTTPException(status_code=404, detail="Conversation not found")
